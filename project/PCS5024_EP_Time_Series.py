...
class ARModel(nn.Module):
    ...
    def encode(self, x: torch.Tensor, x_lengths: torch.Tensor) -> torch.Tensor:
    ...
    def decode(self, h_n: torch.Tensor, y_lengths: torch.Tensor) -> torch.Tensor:
        """Decodes the sequence autoregressively."""
        batch_size = h_n.size(1)
        max_target_length = int(y_lengths.max().item())

        mock_input = torch.zeros(batch_size, max_target_length, 1, device=h_n.device)
    ...
    def forward(
        self, x: torch.Tensor, x_lengths: torch.Tensor, y_lengths: torch.Tensor
    ) -> torch.Tensor:
